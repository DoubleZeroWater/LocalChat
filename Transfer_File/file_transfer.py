...
class MainWindow(wx.Frame):
    def __init__(self, parent, id, title):
        wx.Frame.__init__(self, parent, id, title,
                          size=(800, 600))  # , style=wx.SYSTEM_MENU|wx.MINIMIZE_BOX|wx.CLOSE_BOX|wx.CAPTION)
    ...
    def MsgRecvd(self, event):
        if len(self.msgList):
            print
            "This is message received: ", self.msgList[0]
            if os.name == 'nt':
    ...
    def OnSend(self, event):
        ...
        if data:
            if self.chat:
                ...
                print
                "MAIN WINDOW", userdir[self.chat]
                print
                "MAIN WINDOW", type(userdir[self.chat])
                print
                self.senddata
                sendMsg(self.senddata, self.ipaddr)
                print
                "MAIN WINDOW after send"
            else:
    ...
    def RecvMsg(self, user, data):
        ...
        print
        "MAIN WINDOW win.RecvMsg"
        print
        "MAIN WINDOW", data, user
        import time
        now = time.localtime()
        hour, min, sec = (now.tm_hour, now.tm_min, now.tm_sec)
        if hour < 10:
    ...
    def run(self):
        ...
        try:
            ...
            if filedlg.ShowModal() == wx.ID_OK and filedlg.GetFilename():
                ...
                dirname = filedlg.GetDirectory()
                print
                "IN fileClient FILENAME:", filename
                if filename:
                    path = os.path.join(dirname, filename)
                    sock.send(filename)
                    ack = sock.recv(12)
...
class broadClient(threading.Thread):
    ...
    def run(self):
        ...
        print
        "exit from while and broadClient"
...
def sendMsg(data, ipaddr):
    ...
    try:
        ...
        if ack[0] != "ACK":
            print
            "The message \"%s\" haven't sent successfully" % data
        else:
            print
            "send successfully"
    except socket.error:
...
class recvMsg(threading.Thread):
    ...
    def run(self):
        ...
        while self.flag:
            ...
            print
            "RECVDATA IS ", recvdata, address, username
            print
            type(recvdata)
            print
            "recvMsg receive message successfully"
