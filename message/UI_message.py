...
class Messager(Thread):
        ...
        while not self.connect_end:
            ...
            if recv_data == "VIDEO_REQUEST":
                ...
                try:
                    ...
                    print(
                        'signal received and try to close connection, but failed'
                    )
                finally:
                    sys.exit(0)
                ...
                self.ui.textBrowser.append(
                    '<font color="gray">{}    {}<font>'.format(
                        nick_name, strftime("%H:%M:%S", gmtime())))
    ...
    def main_window(self):
        ...
        self.ui.pushButton.clicked.connect(
            self.send_message)  #将按钮与send_massage方法绑定
        self.ui.pushButton.setShortcut("return")
        self.ui.pushButton_2.clicked.connect(
            self.video_launch)  # 点击视频聊天按钮触发video_connect方法

    # 接受到聊天，调用该方法
    def video_request(self):
        reply = QMessageBox.question(self.window, '视频聊天', "是否接受？",
    ...
    def send_message(self):
        try:
            ...
            if msg:
                ...
                self.ui.textBrowser.append(
                    '<font color="gray">{}    {}<font>'.format(
                        self.nick_name, strftime("%H:%M:%S", gmtime())))
                self.ui.textBrowser.append('{}\n'.format(msg))
                self.ui.textBrowser.moveCursor(
                    self.ui.textBrowser.textCursor().End)
    ...
    def run(self):
        ...
        try:
            ...
            print("读取user.ini文件失败")
            pass

        # 图标
        self.icon = QtGui.QIcon()
        self.icon.addPixmap(QtGui.QPixmap("UI/logo.ico"), QtGui.QIcon.Normal,
                            QtGui.QIcon.Off)
